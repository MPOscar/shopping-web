...
@Component({
  selector: 'collection-shops-table',
  templateUrl: './collection-shops-table.component.html',
  styleUrls: ['./collection-shops-table.component.scss']
})
export class CollectionShopsTableComponent implements OnInit {
  ...
  redrawItems(linkedShops: any) {
    this.items = [];
    linkedShops.forEach(item => {
      if (item && item.shop) {
        let shipping =  item.shop.shippingCountries;
        if (item.shop.shippingCountries === 'Select Countries') {
          shipping = item.shop.countries;
        }
        this.items.push({
          id: item.shop.id,
          name: item.shop.name,
